...
class ContextMaker(object):
    ...
    def get_pmap_by_grp(self, srcfilter, group):
        ...
        dists = []
        totrups = 0
        for srcid, rups, sites, src_group_ids in pmaker.gen_srcid_mag_rups():
            t0 = time.time()
            try:
                poemap = pmaker.make(srcid, src_group_ids, rups, sites, pmap)
            except Exception as err:
                etype, err, tb = sys.exc_info()
                msg = '%s (srcid=%s)' % (str(err), srcid)
...
class PmapMaker(object):
    ...
    def gen_srcid_mag_rups(self):
        """
        :yields: srcid, rups, sites, src_group_ids
        """
        self.mutex_weight = {}  # srcid -> weight
        rows = []  # srcid, mag, rups, src_group_ids
        with self.cmaker.mon('iter_ruptures', measuremem=False):
            for srcid, grp in itertools.groupby(
                    self.group, key=operator.attrgetter('id')):
                srcs = list(grp)
                src_group_ids = srcs[0].src_group_ids
                for src, sites in self.srcfilter(srcs):
                    if self.src_mutex:
                        self.mutex_weight[src.id] = src.mutex_weight
                    for mag, rups in itertools.groupby(
                            src.iter_ruptures(shift_hypo=self.shift_hypo),
                            key=operator.attrgetter('mag')):
                        rows.append(
                            (srcid, mag, list(rups), sites, src_group_ids))
        by_id_mag = operator.itemgetter(0, 1)
        for (srcid, mag), grp in itertools.groupby(
                sorted(rows, key=by_id_mag), key=by_id_mag):
            tups = list(grp)
            sites = tups[0][-2]
            grp_ids = tups[0][-1]
            yield srcid, sum([row[2] for row in tups], []), sites, grp_ids
    ...
    def _update(self, pmap, pm, srcid, src_group_ids):
        if self.rup_indep:
            pm = ~pm
        if not pm:
            return
        if self.src_mutex:
            pm *= self.mutex_weight[srcid]
        for grp_id in src_group_ids:
            if self.src_mutex:
                pmap[grp_id] += pm
            else:
                pmap[grp_id] |= pm

    def make(self, srcid, src_group_ids, rups, sites, pmap):
        ...
        for rup, r_sites, dctx in ctxs:
            if self.fewsites:  # store rupdata
                rupdata.add(rup, srcid, r_sites, dctx)
        ...
        self._update(pmap, poemap, srcid, src_group_ids)
        return poemap
