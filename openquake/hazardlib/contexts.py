...
class ContextMaker(object):
    ...
    def get_pmap(self, src, s_sites, rup_indep=True):
        ...
        for rup, sites in self._gen_rup_sites(src, s_sites):
            ...
            with self.poe_mon:
                pairs = zip(r_sites.sids, self._make_pnes(rup, mean_std, n))
    ...
    def _make_pnes(self, rupture, mean_std, n):
        imtls = self.imtls
        poes = numpy.zeros((n, len(imtls.array), len(self.gsims)))
            ...
            for m, imt in enumerate(imtls):
                if not (hasattr(gsim, 'weight') and gsim.weight[imt] == 0):
                    ...
                    poes[:, imtls(imt), g] = gsim.get_poes(
                        mean_std[g, :, :n, m], imtls[imt], self.trunclevel)
