...
class ContextMaker(object):
    ...
    def get_pmap_from_point(self, src, sctx, rup_indep=True):
        imts = self.imts
        sitecol = sctx.complete
        ...
        rups = sorted(src.iter_ruptures(), key=operator.attrgetter('mag'),
                      reverse=True)
        for mag, group in itertools.groupby(
                rups, key=operator.attrgetter('mag')):
            for rup in group:
                if len(sctx) == 0:
                    continue
                try:
                    with self.ctx_mon:
                        sctx, dctx = self.make_contexts(sctx, rup)
                ...
                with self.gmf_mon:
                    ...
                    ok = numpy.exp(mean_std[:, 0]).max(axis=(0, 2)) >= .01
                    sctx = sitecol.filtered(sctx.sids[ok])
                    if len(sctx) == 0:
                        continue
                with self.poe_mon:
                    for sid, pne in self._make_pnes(
                            rup, sctx.sids, mean_std[:, :, ok]):
    ...
    def get_pmap(self, src, s_sites, rup_indep=True):
        ...
        if hasattr(src, 'location'):
            return self.get_pmap_from_point(src, s_sites, rup_indep)
        imts = self.imts
